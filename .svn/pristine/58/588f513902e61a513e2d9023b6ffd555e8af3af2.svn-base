...
module ApplicationHelper
  ...
  def link_to_permalink(item, title, anchor=nil, style=nil, nofollow=nil,a_title=nil)
    anchor = "##{anchor}" if anchor
    class_attr = "class=\"#{style}\"" if style
    rel_attr = "rel=\"#{nofollow}\"" if nofollow
    a_title_attr = "title=\"#{a_title}\"" if a_title
    "<a href=\"#{item.permalink_url}#{anchor}\" #{rel_attr} #{class_attr} #{a_title_attr}>#{title}</a>"
  end
