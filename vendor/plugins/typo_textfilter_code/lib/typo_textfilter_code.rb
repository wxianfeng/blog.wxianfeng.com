...
注意<typo:code>..</typo:code> 上面有一行空行
        }
      end
      ...
      def self.macrofilter(blog, content, attrib, params, text="")
        ...
        begin
          text = CodeRay.scan(text, lang.to_sym).div(:css => :class)   # text = CodeRay.scan(text, lang.to_sym).span(options)
        rescue
        ...
        "#{text}"  #  "<div class=\"CodeRay\"><pre>#{titlecode}#{text}</pre></div>"
      end
    end
  end
end
