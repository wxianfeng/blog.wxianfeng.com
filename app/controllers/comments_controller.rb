class CommentsController < FeedbackController
  ...
  def create
    ...
    if @comment.save
      # 发邮件
      ago_user_comments = Comment.all(:select => "DISTINCT email",:conditions=>["article_id = ? AND published = 1",params[:article_id]]).select { |ele| !ele.email.blank? }
      ago_user_comments.each do |ele|
        Comment.notify_users_via_email(@comment,ele)
      end
      if request.xhr?
