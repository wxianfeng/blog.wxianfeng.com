...
class Resource < ActiveRecord::Base
  ...
  def create_thumbnail
   ...
   def create_middlenail
    return unless self.mime =~ /image/ or File.exists?(fullpath("middle_#{self.filename}"))
    return unless File.exists?(fullpath("#{self.filename}"))
    begin
      img_orig = MiniMagick::Image.from_file(fullpath(self.filename))
      img_orig = img_orig.resize('470x470')
      img_orig.write(fullpath("middle_#{self.filename}"))
    rescue
      nil
    end
  end
